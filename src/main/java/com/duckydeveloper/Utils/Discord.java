...
import static com.duckydeveloper.Globals.setMinecordLogs;

public class Discord {
    public static void checkGuildPermissions(JDA jda) {
        Guild guild = jda.getGuildById(Globals.getGuild());

        if (guild == null) {
            Bukkit.getLogger().severe("Bot is not on a server with ID: " + Globals.getGuild());
        } else if (guild.getSelfMember().hasPermission(Permission.ADMINISTRATOR)) {
            Bukkit.getLogger().fine("Bot has administrator permissions on the server with ID: " + Globals.getGuild());
        } else {
            Bukkit.getLogger().severe("Bot does not have administrator permissions on the server with ID: " + Globals.getGuild());
        }
    }
    ...
    public static void log(String title, String text, Color color) {
        ...
        Guild guild = DcMain.jda.getGuildById(Globals.getGuild());
        ...
        if (minecordLogsChannel != null) {
            setMinecordLogs(minecordLogsChannel.getId());
            ...
            if (!guild.getSelfMember().hasPermission(Permission.MANAGE_CHANNEL)) {
                ...
                guild.createTextChannel("minecord-logs")
                        .queue(createdChannel -> {
                            setMinecordLogs(createdChannel.getId());
